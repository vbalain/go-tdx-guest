...
type TDQuoteBodyPolicy struct {
	...
	EnableTdMigratableCheck bool                   `protobuf:"varint,13,opt,name=enable_td_migratable_check,json=enableTdMigratableCheck,proto3" json:"enable_td_migratable_check,omitempty"` // if true, check that the MIGRATABLE bit is 0 in TDAttributes.
	unknownFields           protoimpl.UnknownFields
	sizeCache               protoimpl.SizeCache
}
...
const file_checkconfig_proto_rawDesc = "" +
	"\n" +
	"\x11checkconfig.proto\x12\vcheckconfig\"\x99\x01\n" +
	"\x06Policy\x12>\n" +
	...
	" \x01(\fR\n" +
	"reportData\x12\x1a\n" +
	"\tany_mr_td\x18\v \x03(\fR\aanyMrTd\x121\n" +
	"\x15enable_td_debug_check\x18\f \x01(\bR\x12enableTdDebugCheck\x12;\n" +
	"\x1aenable_td_migratable_check\x18\r \x01(\bR\x17enableTdMigratableCheck\"\x96\x01\n" +
	"\vRootOfTrust\x12%\n" +
...
var (
	...
	file_checkconfig_proto_rawDescData []byte
)

func file_checkconfig_proto_rawDescGZIP() []byte {
	file_checkconfig_proto_rawDescOnce.Do(func() {
		file_checkconfig_proto_rawDescData = protoimpl.X.CompressGZIP(unsafe.Slice(unsafe.StringData(file_checkconfig_proto_rawDesc), len(file_checkconfig_proto_rawDesc)))
	})
...
func file_checkconfig_proto_init() {
	...
	out := protoimpl.TypeBuilder{
		File: protoimpl.DescBuilder{
			...
			RawDescriptor: unsafe.Slice(unsafe.StringData(file_checkconfig_proto_rawDesc), len(file_checkconfig_proto_rawDesc)),
			NumEnums:      0,
			NumMessages:   5,
			NumExtensions: 0,
			NumServices:   0,
